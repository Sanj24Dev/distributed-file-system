...
def replicate(filename):
	...
	port = nodes.fileserverB_PORT
	server_ip = nodes.fileserverB_IP
	...
	port = nodes.fileserverC_PORT
	server_ip = nodes.fileserverC_IP
...
def main():
	...
	server_addr = nodes.fileserverA_IP
	server_port = nodes.fileserverA_PORT
	...
	while 1:
		...
		if recv_msg != "" and "CHECK_VERSION" not in recv_msg:
			# parse the message
			filename = msg[0]	# file path to perform read/write on
			print ("Filename: " + filename)
			RW = msg[1]			# whether its a read or write
			print ("RW: " + RW)
			text = msg[2]		# the text to be written (this text is "READ" for a read and is ignored)
			print ("TEXT: " + text)
			...
			client_filename = msg[1]			# parse the version number to check
